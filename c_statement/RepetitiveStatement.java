...
//		}while(input != answer); //정답이 아니면 반복
		...
		outer : for(int i = 2; i <= 9; i++) {
			for(int j = 1; j <= 9; j++) {
				if(j == 5) {
//					break; //가장 가까운 반복문 하나를 빠져나간다. (i*4까지 반복)
					break outer; //outer라는 이름의 반복문을 빠져나간다. (2*1,2,3,4 까지)
//					continue; //가장 가까운 반복문의 현재 반복회차를 빠져나간다. (*5마다 제외)
